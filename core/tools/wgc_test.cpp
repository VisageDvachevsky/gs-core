...
int main(int argc, char** argv) {
    ...
    uint32_t window_handle = 0;  // 0 = foreground window
    if (argc > 1) {
        window_handle = std::stoul(argv[1], nullptr, 16);  // Parse as hex
    }
    ...
    while (running) {
        ...
        if (elapsed >= 1.0f) {
            ...
            std::cout << std::format("[{:.1f}s] FPS: {:.1f} | Captured: {} | Skipped: {} | Latency: {:.3f}/{:.3f}/{:.3f} ms (min/avg/max)\n",
    ...
    if (frames_captured > 0) {
        ...
        if (avg_fps >= 50) {
            std::cout << "\n\u2705 SUCCESS: Achieved 50+ FPS!\n";
        } else {
            std::cout << "\n\u26A0\uFE0F WARNING: FPS below 50. Check if target window is active.\n";
        }
    }
